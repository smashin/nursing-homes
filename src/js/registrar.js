...
App = {
    ...
    renderContent: function () {
        ...
        App.contracts.RealEstateDeals
            ...
            .then(function (instance) {
                ...
                var event = instance.DealStateRegistrationRequested({});
                // watch for changes
                event.watch(function (error, result) {
                    if (!error) {
                        ...
                        var developer = result.args.developer.toString().toLowerCase();
                        return dealsInstance.getDealFilesJson.call(dealId, {from: App.address})
                            .then(files => {
                                var files = JSON.parse(files);
                                var dealHtml = `<div class="panel panel-warning j-deal" data-id="${dealId}">`;
                                dealHtml += `<div class="panel-heading"><h3 class="panel-title">Запрос #${dealId}. Застройщик ${App.accountsInfo[developer].name}</h3></div>`
                                ...
                                dealHtml += '<div>' +
                                    '<button class="btn btn-success j-deal-approve" data-loading-text="Идет сохранение...">Зарегистрировать</button>&nbsp;' +
                                    '<button class="btn btn-danger j-deal-decline" data-loading-text="Идет сохранение...">Отказать</button>' +
                                    '</div>';
                                dealHtml += '</div></div>';
    ...
    handleApproveDeal: function () {
        if (confirm(`Вы действительно зарегистрировать сделку?`)) {
            ...
            App.contracts.RealEstateDeals.deployed()
                .then(function (instance) {
                    ...
                    return dealsInstance.approveStateRegistration(id, {from: address});
                })
                .then(result => {
                    $button.button('reset');
                    var logs = result.logs[0];
                    if (logs && logs.event == 'DealRegistrationApproved') {
                        $counter.text(parseInt($counter.text()) - 1);
    ...
    handleDeclineDeal: function () {
        if (confirm(`Вы действительно отказать в регистрирации сделки?`)) {
            var $button = $(event.target);
            var $counter = $('.j-deals-count');
            $button.button('loading');
            var dealsInstance;
            ...
            App.contracts.RealEstateDeals.deployed()
                .then(function (instance) {
                    dealsInstance = instance;
                    return dealsInstance.rejectStateRegistration(id, 'Причина отказа', {from: address});
                })
                .then(result => {
                    $button.button('reset');
                    var logs = result.logs[0];
                    if (logs && logs.event == 'DealRegistrationRejected') {
                        $counter.text(parseInt($counter.text()) - 1);
                        $button.closest('.j-deal').remove();
                    }
                    else {
